<script lang="ts">
    ...
    import { warning, error, success } from "$/utils/alert";
    import { fade } from "svelte/transition";
    import { parseColumnName, type TableName } from "$/utils/tables";
    ...
    export let selectSource = `/${name}/info/`;

    const selectHandler = () => {
        currentRow = undefined;
        selectedKeys.length = 0;

        console.log({ currentPage: currentPage, startIndex: startIndex, count: count, maxPage: maxPage, total: total });

        getJSON(selectSource, {
            column: parseColumnName(searchBy),
            key: query,
            match: fullMatch ? "eq" : "like",
            sort: parseColumnName(sortBy),
            order: desc ? "DESC" : "ASC",
            ...
            rows = preprocess(columns, data.data.data);
        });
    };
    ...
    const deleteHandler = () => {
        postJSON(`/${name}/remove/`, [currentRow![primaryKey]]).then((data) => {
            if (data.code !== 200) {
                error("删除内容失败!");
                return;
            }
    ...
    const updateHandler = () => {
        postJSON(`/${name}/update/`, currentRow).then((data) => {
            if (data.code !== 200) {
                error("更新内容失败!");
                return;
            }
    ...
    const insertHandler = () => {
        postJSON(`/${name}/add/`, currentRow).then((data) => {
            if (data.code !== 200) {
                error("插入内容失败!");
                return;
            }
    ...
    const multipleDeleteHandler = () => {
        postJSON(`/${name}/remove/`, selectedKeys).then((data) => {
            if (data.code !== 200) {
                error("删除内容失败!");
                return;
            }
...
<div class="flex flex-col justify-center gap-4 mt-4 mx-auto">
    <!-- select -->
    <div class="flex justify-start lg:justify-center items-center bg-transparent gap-4 overflow-auto">
        <!-- search by -->
        <div class="tooltip" data-tip="搜索依据">
        ...
        <div class="tooltip tooltip-primary" data-tip="排序依据">
        ...
        <!-- limitation -->
        <div class="form-control tooltip" data-tip="共 {total} 条记录">
            <label class="cursor-pointer label gap-2">
                <!-- why new line? -->
        ...
        {#if maxPage > 1}
            <div class="form-control tooltip" data-tip={`${1} - ${maxPage}`}>
    ...
    <table class="table table-md lg:table-lg mx-auto" in:fade>
        ...
        <tbody>
            {#each rows as row (row[primaryKey])}
                <tr class="hover:bg-base-200 transition text-center">
                    <!-- 内容 -->
                    {#each Object.entries(columns) as [key, column] (key)}
                        {#if row[key]}
